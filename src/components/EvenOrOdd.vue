    ...
    <v-form>
      <v-row>
        <v-col cols="4">
          ...
          ></v-text-field>
          <v-btn
              @click="checkPrime() + checkPrime2() + oddEven() + printFactor() + sumPrimeF() + amicable() + coprime()"
  ...
  methods: {
    ...
    checkCoprime() {
      ...
      for (let x = 2; x < small; x++) {
        if (this.number1 % x === 0 && this.number2 % x === 0) {
          return false
        }
      }
      return true
    ...
    amicable() {
      const {sum, dArray} = this.sumAmicable(this.number1)
      console.log('sum', sum);
      ...
      if (this.sumAmicable(this.number1) === this.number2 && this.sumAmicable(this.number2) === this.number1) {
        ...
        this.message7 = this.number1 + ' and ' + this.number2 + ' are not amicable '
      }
    },
